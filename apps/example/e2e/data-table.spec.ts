...
test.describe('data tables', () => {
  ...
  test('should render table with sticky header', async ({ page }) => {
    ...
    const row4 = sticky.locator('table tbody tr:nth-child(5)');
    ...
    let classes = await mainHead.getAttribute('class');
    expect(classes).toContain('_sticky__header_');
    expect(classes).not.toContain('_stick__top_');

    await page.evaluate(() => window.scrollTo(0, 0));

    classes = await mainHead.getAttribute('class');
    expect(classes).not.toContain('_stick__top_');
  });
});
