...
class Locale {
  ...
  constructor(
    ...
    const { firstDayOfWeek, id, masks, monthCacheSize, pageCacheSize }
  ...
  fromDate(date: Date | null, { mask, type }: Partial<DateOptions> = {}) {
    switch (type) {
      case 'number':
  ...
  getHourLabels() {
    return getHourDates().map(d => this.formatDate(d, this.masks.hours));
  }
