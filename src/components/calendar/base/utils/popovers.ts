...
function removeHandlers(target: Element | ComponentPublicInstance | string) {
...
function addHandlers(target: Element | ComponentPublicInstance | string, opts: Partial<PopoverOptions>) {
  const el = resolveEl(target);
  if (el == null)
    return;
  const remove: ((...args: any[]) => void)[] = [];
