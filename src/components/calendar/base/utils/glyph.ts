...
const _defaultProfile = { base: {}, end: {}, start: {} };
...
function normalizeConfig(
  ...
  const result = defaultsDeep(
    root,
    { end: root.startEnd, start: root.startEnd },
    defaultProfile,
  ) as Profile<any>;
...
export class HighlightRenderer implements GlyphRenderer<Highlight> {
  ...
  render(
    { data, onEnd, onStart }: DateRangeCell<Attribute>,
    glyphs: Record<string, Highlight[]>,
  ) {
    const { highlight, key } = data;
    if (!highlight)
      return;
    const { highlights } = glyphs;
    const { base, end, start } = highlight;
    if (onStart && onEnd) {
      highlights.push({
        ...start,
