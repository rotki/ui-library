...
- `pnpm run test:e2e` - Build and run tests against production build
- `pnpm run test:e2e:dev` - Run against dev server (faster iteration)
- `pnpm run test:e2e:ui` - Run with Playwright UI (for debugging)
- `pnpm run test:e2e:docker` - Run tests in Docker container
